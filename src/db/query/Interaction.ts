...
export default class {
  ...
  public async queryAccess(timelineId: string): Promise<IAccess[]> {
    const query = await this._aqlQuery`FOR doc IN interaction FILTER doc.timelineId == ${timelineId} RETURN doc`;
    const result = await query.all();
    return result;
  }

  public async queryHasPublicKey(publicKey: string): Promise<boolean> {
    const query = await this._aqlQuery`FOR doc IN interaction FILTER doc.publicKey == ${publicKey} RETURN doc._key`;
    const result = await query.next();
    return !!result;
  }

  public async queryAccessWithPublicKey(timelineId: string, publicKey: string): Promise<IAccess & { _key: any }> {
    const query = await this._aqlQuery`FOR doc IN interaction FILTER doc.timelineId == ${timelineId} AND doc.publicKey == ${publicKey} RETURN doc`;
    const result = await query.next();
    return result;
  }
}
